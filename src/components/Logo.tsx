...
export function Logo3MR({ className = '', size = 'md', showBadge = false }: LogoProps) {
  ...
  if (showBadge) {
    return (
      <div 
        className={`inline-flex items-center justify-center bg-white dark:bg-slate-900 border border-gray-200/90 dark:border-slate-800 shadow-sm hover:shadow transition-colors ${badgeWrapper[size]}`}
